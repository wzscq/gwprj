...
func (controller *ProjectController)downloadReportNew(c *gin.Context){
	log.Println("ProjectController start downloadReportNew")
	...
	if err := c.ShouldBindHeader(&header); err != nil {
		...
		rsp:=common.CreateResponse(common.CreateError(common.ResultWrongRequest,nil),nil)
		c.IndentedJSON(http.StatusOK, rsp)
		log.Println("end downloadReportNew with error")
		return
	}	
	...
	if rep.SelectedRowKeys==nil || len(*rep.SelectedRowKeys)==0 {
		rsp:=common.CreateResponse(common.CreateError(common.ResultWrongRequest,nil),nil)
		c.IndentedJSON(http.StatusOK, rsp)
		log.Println("downloadReportNew error：request list is empty")
		return
	}
...
func (controller *ProjectController) Bind(router *gin.Engine) {
	...
	router.POST("project/downloadReport", controller.downloadReportNew)
}